(function(){
  ...
  async function adminCatalog(type){assertConfigured();const table=type==='shop'?'shop_products':'menu_items';const {data,error}=await client.from(table).select('*').order('sort_order',{ascending:true}).order('name',{ascending:true});if(error)throw error;return (data||[]).map(normalizeProduct).filter(item=>item.category!=='__system'&&(type!=='pickup'||item.active));}
