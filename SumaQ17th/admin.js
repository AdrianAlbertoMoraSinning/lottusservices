...
function renderReservations(){$('reservationsBody').innerHTML=reservations.length?reservations.map(r=>`<tr><td>${esc(fmt(r.created_at))}<br><small>${esc(r.id)}</small></td><td>${esc(r.reservation_date)}<br>${esc(r.reservation_time)}</td><td>${esc(r.first_name+' '+(r.last_name||''))}</td><td>${r.adults} adults<br>${r.minors} minors<br>${r.party_size} total</td><td>${esc(r.phone)}<br>${esc(r.email)}</td><td>${money(r.deposit_amount)}<br>${esc(r.payment_status)}</td><td>${esc(r.table_label||'Unassigned')}</td><td>${statusSelect('reservations',r.id,r.status,['New','Awaiting deposit','Booked','Confirmed','Seated','Completed','No-show','Cancelled','Archived'])}</td><td>${esc(r.notes)}${r.staff_note?`<br><small>Staff: ${esc(r.staff_note)}</small>`:''}</td><td><button class="mini-btn delete-record" data-table="reservations" data-id="${r.id}">Delete</button></td></tr>`).join(''):'<tr><td colspan="10">No central reservations yet.</td></tr>'}
...
async function refreshAll(){
  try{
    $('reservationsBody').innerHTML='<tr><td colspan="10">Loading central data…</td></tr>';$('eventsBody').innerHTML='<tr><td colspan="7">Loading central data…</td></tr>';
...
$('exportReservations').onclick=()=>download('sumaq-reservations.csv',['ID,Created,Date,Time,First Name,Last Name,Phone,Email,Adults,Minors,Party Size,Deposit,Payment,Table,Status,Guest Notes,Staff Note',...reservations.map(r=>[r.id,r.created_at,r.reservation_date,r.reservation_time,r.first_name,r.last_name,r.phone,r.email,r.adults,r.minors,r.party_size,r.deposit_amount,r.payment_status,r.table_label,r.status,r.notes,r.staff_note].map(csv).join(','))].join('\n'));
