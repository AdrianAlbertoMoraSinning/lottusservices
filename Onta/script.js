...
$('#payDemo')?.addEventListener('click',()=>{const cfg=window.ONTA_CONFIG||{}; const total=COP(cart.reduce((a,b)=>a+b.price,0)); const detail=cart.map(it=>`${it.name} ${it.size} ${it.color}`).join(', ')||'Asesoría de compra'; const subject=encodeURIComponent('Pedido ONTA STORE - '+total); const body=encodeURIComponent(`Hola ONTA STORE, quiero pagar este pedido:
...
Nombre:
WhatsApp:
Dirección:`); window.location.href=`mailto:${cfg.email||'ventas@onta.com.co'}?subject=${subject}&body=${body}`});

$('#demoCart')?.addEventListener('click',()=>{ if(!products.length) return; cart=[]; products.slice(0,3).forEach((p,i)=>cart.push({...p,size:p.sizes?.[0]||'',color:p.colors?.[0]||'',qty:1})); saveCart(); document.getElementById('pago')?.scrollIntoView({behavior:'smooth'}); setTimeout(openCart,450); });
