...
exports.handler=async function(event){
  ...
  for(const strategy of ["mobile","desktop"]){
    const qs=new URLSearchParams({url:u.toString(),strategy});for(const c of ["performance","accessibility","best-practices","seo"])qs.append("category",c);if(key)qs.set("key",key);
  ...
  return jsonResponse(200,{ok:results.some(x=>x.ok),mode:"READ_ONLY",engine:"Octon PageSpeed Live Audit v1.2",target:u.toString(),results,generatedAt:new Date().toISOString()});
};
