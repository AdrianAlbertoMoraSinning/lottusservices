...
exports.handler=async function(event){
  ...
  const start=Date.now(),r=await safeFetch(target,{headers:{"user-agent":"Octon-Portal-Snapshot/1.3"}}),html=await r.text(),h=Object.fromEntries(r.headers.entries());
  ...
  return jsonResponse(200,{ok:r.ok,mode:"READ_ONLY",engine:"Octon Portal Snapshot v1.3",status:r.status,finalUrl:r.url,responseMs:Date.now()-start,bytes:Buffer.byteLength(html),
