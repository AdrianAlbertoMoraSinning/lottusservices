...
exports.handler=async function(event){
  ...
  let u;try{u=new URL(target)}catch{return jsonResponse(400,{ok:false,error:"Invalid URL"})}
  const start=Date.now(),r=await fetch(u,{redirect:"follow",headers:{"user-agent":"Octon-Portal-Snapshot/1.2"}}),html=await r.text(),h=Object.fromEntries(r.headers.entries());
  ...
  const add=(severity,dimension,title,evidence,recommendation)=>findings.push({id:`snapshot-${findings.length+1}`,severity,dimension,title,evidence,operationalImpact:null,commercialImpact:null,recommendation,affectedFiles:["index.html"],tests:[],rollback:null,confidence:.95,requiresHumanReview:false,sources:[]});
  ...
  return jsonResponse(200,{ok:r.ok,mode:"READ_ONLY",engine:"Octon Portal Snapshot v1.2",status:r.status,finalUrl:r.url,responseMs:Date.now()-start,bytes:Buffer.byteLength(html),
    ...
    securityHeaders:Object.fromEntries(["strict-transport-security","content-security-policy","x-content-type-options","referrer-policy","permissions-policy"].map(k=>[k,h[k]||null])),
