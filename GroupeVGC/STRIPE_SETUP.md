# Groupe VGC v10 — Stripe activation checklist

This build replaces the demo card form with Stripe-hosted Checkout for final invoice payments.

## 1. Stripe account

The Groupe VGC owner must create/activate the Stripe account directly with the business/legal representative and payout bank information. Do not put banking credentials, Stripe passwords, MFA codes, secret API keys, or webhook secrets in GitHub.
...
For the current `lottusservices` monorepo where Groupe VGC lives in `/rbmoving`, set:

- Package directory: `rbmoving`
- Publish directory: `rbmoving`
- Functions directory: `rbmoving/netlify/functions`

The included `rbmoving/netlify.toml` expresses the same paths relative to the repository root.

## 4. Netlify environment variables

Add these in Netlify > Groupe VGC > Project configuration > Environment variables:

- `STRIPE_SECRET_KEY` = Stripe sandbox/test secret key initially; later live secret/restricted key
- `STRIPE_WEBHOOK_SECRET` = signing secret for the Stripe webhook endpoint
- `SUPABASE_URL` = existing Groupe VGC Supabase project URL
...
- `SITE_URL` = `https://[DOMAINE_VGC_A_CONFIGURER]`
...
`https://[DOMAINE_VGC_A_CONFIGURER]/.netlify/functions/stripe-webhook`
