# Groupe VGC — Supabase activation

1. Create a dedicated Supabase project for Groupe VGC.
2. Open SQL Editor and run `schema.sql` once.
3. In Authentication > Users, create the Groupe VGC administrator user (email + password).
4. Copy that user's UUID and run:

```sql
insert into public.admin_users(user_id, display_name)
values ('PASTE_AUTH_USER_UUID', 'Groupe VGC Admin')
