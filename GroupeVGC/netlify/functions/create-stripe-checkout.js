...
exports.handler = async (event) => {
  ...
  try {
    ...
    const siteUrl = env('SITE_URL', 'https://[DOMAINE_VGC_A_CONFIGURER]').replace(/\/$/, '');
    ...
    const params = {
      ...
      'line_items[0][price_data][product_data][name]': `Groupe VGC Invoice ${invoice.invoice_number}`,
      'line_items[0][price_data][product_data][description]': invoice.booking_reference ? `Final invoice for booking ${invoice.booking_reference}` : 'Final Groupe VGC invoice',
    ...
    return json(500, { error: 'Unable to start secure payment right now. Please contact Groupe VGC.' });
  }
};
